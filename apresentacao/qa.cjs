...
async function run(){
 ...
 try {
  ...
  for(const tab of ['trabalho','metodo','resultados','ensino','fontes']){
    await page.locator(`nav [data-tab="${tab}"]`).click();assert(await page.locator('#'+tab).isVisible());
  ...
  await page.locator('#count-slider').fill('15');await page.locator('#count-slider').dispatchEvent('input');assert.equal(await page.locator('#count-result').innerText(),'43.589.145.600');
